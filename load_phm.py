...
def divide_signal(df, segment_length,label,dic ,idx, seg_num =None ):
    ...
    for segment in range(n_segments):
        dic[idx] = {
            'signal': df[3][seg_num * segment:segment * seg_num+ segment_length].values.reshape(1024,1), 
            # 'label': df.iloc[i,2],
            'label' : label
        }
        idx += 1
...
def load_phm(file_name):
    ...
    for i, file in enumerate(tqdm(os.listdir(os.path.join(DATA_PATH,file_name)))):
        ...
        dic,idx = divide_signal(df, dic =dic,idx=idx,segment_length=1024,seg_num=2000,label=label)
